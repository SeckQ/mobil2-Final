...
const generateTranslatePrompt = (textToTranslate: string, selectedLanguage: string) => {
    return `Tu tienes un rol de traductor de idiomas y requiero que conviertes este texto a ${selectedLanguage}: ${textToTranslate}`
    //return ` Eres un traductor de números en texto a numero binario, entonces quiero que traduzcas este número en texto a binario: ${textToTranslate}`
}
...
app.post('/translate', async (req, res) => {
    ...
    const completion = await openai.createCompletion({
        model: 'gpt-3.5-turbo-instruct',
        prompt: generateTranslatePrompt(prompt, language),
        temperature: 0.1
    })
...
//Convertir a binario
const generateTextBinaryPrompt = (textToConvert: string) => {
...
app.post('/textbinary', async (req, res) => {
    const {prompt} = req.body
    const completion = await openai.createCompletion({
        model: 'gpt-3.5-turbo-instruct',
        prompt: generateTextBinaryPrompt(prompt),
        temperature: 0.1
    })
...
const generateCountPrompt = (textToCount: string) => {
    return `Eres un asistente y necesito que cuentes las vocales que hay en el siguiente texto: "${textToCount}"`
}

app.post('/contarvocales', async (req, res) => {
    const {prompt} = req.body
    const completion = await openai.createCompletion({
        model: 'gpt-3.5-turbo-instruct',
        prompt: generateCountPrompt(prompt),
        temperature: 0.1
    })
